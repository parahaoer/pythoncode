...
def getFileShorname(str):
    if('\\' in str):
        filepath, fullflname = os.path.split(str)
        return fullflname
    else:
        return str
...
for line in open(r'covenant_dcsync_all_2019-10-27064128.json'):
    ...
    source = getFileShorname(source)
    target = getFileShorname(target)
    application = getFileShorname(application)
    ...
    wb.save('outputWithShortname.xls')
    i = i + 1
